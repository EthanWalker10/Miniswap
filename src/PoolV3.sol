...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    /**
     * @note Pool 的核心数据结构
     * @note 占据第一个 slot： Slot0 public override slot0;
     */
    struct Slot0 {
        // the current price
        ...
        uint16 observationIndex; // 这些数据用于计算时间加权平均价格（TWAP）等指标。observationIndex 用于标识下一个写入观察值的位置。
        // the current maximum number of observations that are being stored
        uint16 observationCardinality; // 当前存储的观察数据的数量
        // the next maximum number of observations to store, triggered in observations.write
        uint16 observationCardinalityNext; // 下一个最大可存储的观察数据数量
        // the current protocol fee as a percentage of the swap fee taken on withdrawal
        // represented as an integer denominator (1/x)%
        uint8 feeProtocol; // 从每笔交易中收取的费用比例。通过 1/x 的形式表示。例如 feeProtocol = 10，表示协议收取 1% 的交易费。
        // whether the pool is locked
        bool unlocked; // 表示当前池是否被锁定。当池处于锁定状态时，不能进行交易或流动性操作. 通常在合约的升级、调度或安全性考虑时使用。
    }
